...
describeWithMeta(CASE_META, () => {
  ...
  await frame.getByRole('button', { name: '新建考试' }).click();
  ...
  await frame.getByRole('tablist').filter({ hasText: '中国虚拟现实应用状况 .pdf' }).getByRole('checkbox').click();
  await frame.getByRole('button', { name: '确定', exact: true }).click();

  await frame.getByRole('button', { name: '添加题型' }).click();
  await frame.locator('div:nth-child(2) > .exam-menu-item__content > .QuestionType__option-item').first().click();
  await frame.getByRole('button', { name: '添加题型' }).click();
  await frame.getByText('判断题').first().click();
  await frame.getByRole('button', { name: '添加题型' }).click();
  await frame.getByText('填空题').first().click();
  await frame.getByRole('button', { name: '添加题型' }).click();
  await frame.getByText('简答题').first().click();
  await frame.getByText('简单考查记忆，题干简洁，单知识点').click();
