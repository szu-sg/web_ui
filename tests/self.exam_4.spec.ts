...
describeWithMeta(CASE_META, () => {
  ...
  await frame.getByRole('tablist').filter({ hasText: '恰逢惊蛰' }).getByRole('checkbox').click();
  await frame.getByRole('tablist').filter({ hasText: '南京邮电大学出国手续办理须知 .doc' }).getByRole('checkbox').click();
  await frame.getByRole('tablist').filter({ hasText: '南京邮电大学会议审批及会议费预算表 .docx' }).getByRole('checkbox').click();
